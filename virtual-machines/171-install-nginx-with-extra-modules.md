...
mkdir nginxplus
cd /tmp/nginxplus
curl http://www.openssl.org/source/openssl-1.0.1t.tar.gz | tar xvz
...
```
rvmsudo passenger-install-nginx-module --auto --prefix=/usr/local/nginx --nginx-source-dir=/tmp/nginxplus/nginx-1.10.0 --extra-configure-flags="--with-http_spdy_module --with-openssl=/tmp/nginxplus/openssl-1.0.1t --add-module=/tmp/nginxplus/headers-more-nginx-module-0.20" --languages ruby
```
...
Type
```
--with-http_spdy_module --with-openssl=/tmp/nginxplus/openssl-1.0.1t --add-module=/tmp/nginxplus/headers-more-nginx-module-0.20
```
