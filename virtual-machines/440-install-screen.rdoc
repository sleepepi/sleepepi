...
NOTE: If you get <tt>fatal: could not create work tree dir 'screen'.: Permission denied</tt> that means you forgot to run:
...
NOTE: If you get <tt>Do you wish to trust this .rvmrc file? (/usr/local/production/screen/.rvmrc)</tt> then type <tt>yes</tt>
...
NOTE: The contents of the following files will need to be copied from *epipro01* since they are not in version control and contain pseudo-randomly generated strings
  ...
  scp username@epipro01.dipr.partners.org:/usr/local/production/screen/config/initializers/devise.rb /usr/local/production/screen/config/intializers/devise.rb
  scp username@epipro01.dipr.partners.org:/usr/local/production/screen/config/initializers/omniauth.rb /usr/local/production/screen/config/intializers/omniauth.rb
  scp username@epipro01.dipr.partners.org:/usr/local/production/screen/config/initializers/secret_token.rb /usr/local/production/screen/config/intializers/secret_token.rb
...
<b>Screen is not available through the sleepepi router!</b>
